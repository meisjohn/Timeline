...
parser = argparse.ArgumentParser(
    description="Generate interactive HTML timelines from an Excel schedule."
)
parser.add_argument(
    "input_file",
    nargs="?",
    default="data/events.xlsx",
    help="Path to the source Excel file (default: data/events.xlsx)",
)
parser.add_argument(
    "--past",
    type=int,
    default=30,
    help="Days in the past for the rolling window (default: 30)",
)
parser.add_argument(
    "--future",
    type=int,
    default=90,
    help="Days in the future for the rolling window (default: 90)",
)
...
phase_map = {
    p: NON_RED_PALETTE[i % len(NON_RED_PALETTE)] for i, p in enumerate(unique_phases)
}
...
views = [
    {"name": "Full_Timeline", "range": [data_min, data_max]},  # Forces tight bounds
    {
        "name": "Rolling_Window",
        "range": [
            today - timedelta(days=ROLLING_WINDOW_PREVIOUS_DAYS),
            today + timedelta(days=ROLLING_WINDOW_FUTURE_DAYS),
        ],
    },
]
...
for view in views:
    ...
    if view["name"] == "Rolling_Window":
        ...
        view_event_order = [
            e for e in event_order if e in valid_events or e == "ALL EVENTS"
        ]
    else:
        view_df = plot_df
    ...
    p.add_layout(
        BoxAnnotation(
            left=box_left - timedelta(days=200),
            right=today,
            fill_color="white",
            fill_alpha=0.75,
            level="overlay",
    ...
    if "ALL EVENTS" in view_event_order:
        p.hbar(
            y=["ALL EVENTS"],
            left=view["range"][0] - (view["range"][1] - view["range"][0]),
            right=view["range"][1] + (view["range"][1] - view["range"][0]),
            height=1,
            fill_color=None,
            line_color="#000000",
            line_width=2,
    ...
    for status, style in STATUS_MAP.items():
        ...
        dummy = p.hbar(
            y=[view_event_order[0]] if view_event_order else [],
            left=[today],
            right=[today],
            visible=False,
            fill_color=style["fill"] or "white",
            line_color=style["border"],
            line_width=style["width"],
        )
        items.append(LegendItem(label=f"Status: {status}", renderers=[dummy]))
    p.add_layout(
        Legend(items=items, location="bottom_center", orientation="horizontal"), "below"
    )
